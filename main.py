...
class _OfflineLLMProvider(LLMProvider):
    """Lightweight local heuristics for sample-mode runs without API keys."""
...
def run_pipeline(
    ...
    dedup_index = DedupIndex() if use_sample_data else prune_old_entries(load_dedup_index(dedup_path), today=today)
